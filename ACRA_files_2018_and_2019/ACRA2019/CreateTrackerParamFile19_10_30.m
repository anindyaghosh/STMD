...
% 19_10_01: Had to set turn_threshold to 0. The main reason for creating
% this is that the model script was significantly changed.

% 19_10_30: Adding parameters for ZB_fac implementation

% Use the same fixed vel belief as previously
% a=load('D:\simfiles\conf2\trackerinfo\TrackerParamFile19_10_01_kf.mat');
a=load('E:\PHD\TEST\TrackerParamFile19_10_03.mat');

belief_vel_fixed = a.belief_vel_fixed;
...
fixed.turn_distance_threshold = 5; % how many degrees left/right of centre before a turn should be initiated
...
fixed.prob.output_threshold_scalar = 0.0013;
fixed.prob.acquisition_threshold = 1e-5;
...
fixed.fac.output_turn_threshold = 0.001;

fixed.kf.output_threshold_scalar = fixed.prob.output_threshold_scalar;
fixed.kf.output_integration = 4;
fixed.kf.P_post_initial = eye(4);
fixed.kf.Q_initial = 5*eye(4);
fixed.kf.R_initial = 0.01*eye(4);
fixed.kf.certainty_threshold = 9999; % Effectively ignoring certainty
fixed.kf.acquired_ignore_distance = 5;
fixed.kf.max_certainty=40;

fixed.constgain.output_integration = 1;
fixed.constgain.width=8; % how far the gain field extends in both directions from the midline
fixed.constgain.value=25^2; % Gain to be applied to outputs in the gain field. 1 elsewhere.
fixed.constgain.output_threshold_scalar = fixed.prob.output_threshold_scalar;

fixed.none.output_integration = 1;
fixed.none.output_threshold_scalar = fixed.prob.output_threshold_scalar;

fixed.ZB_fac.output_integration = 1;
fixed.ZB_fac.Threshold = 0.1;
fixed.ZB_fac.output_threshold = 0;
fixed.ZB_fac.wb = 0.05;
fixed.ZB_fac.FacilitationGain = 25;
fixed.ZB_fac.RC_wb = 5;
fixed.ZB_fac.parameter.RC_fac_wb = 1;
fixed.ZB_fac.Threshold = 0.1;
fixed.ZB_fac.Ts = 0.05;
fixed.ZB_fac.Facilitation_sigma=5/2.35482;
...
outer_var.ranges.tracker = {...
    'facilitation','facilitation_dark',...
    'constgain','constgain_dark',...
    'ZB_fac','ZB_facdark'
    };
...
save('E:\PHD\TEST\TrackerParamFile19_10_30.mat',...
