...
if(make_histograms)
    ...
    trackerparams.pos_std = 1*output_integration; % Gaussian around the observed location
    ...
    [trackerparams.Y,trackerparams.X] = ndgrid(1:vfov,1:hfov); % Grid coordinates for screen
    ...
    for x=1:5
        for y=1:5
            ...
            if(x == 3 && y == 3)
                dmax(x,y)= norm([0.5 0.5]);
            else
                dmax(x,y)= norm([xdist+0.5 ydist+0.5]);
            end
        end
    end
    
    % Identify which velocities should be summed across for any
    trackerparams.pos_velocities = cell(5,5);
    
    trackerparams.vmin = NaN(5,5);
...
%                             prob_R_v(p) = sum(trackerparams.prob_R_v(resp_bin,:)' .* predict.vel);
                            ...
                            prob_R_pos(pix_ix) = Correl_Patch19_09_08(prob_R_pos_kernel,belief.pos,pix_ix);
                            return
                        end
                    ...
                    if(~acquired)
                        ...
                        if(max_output > trackerparams.output_threshold)
                            ...
                            if(exist('last_detection','var'))
                                ...
                                % detection
                                position_prediction_kernel = NaN(5);
                                [Y_mini,X_mini] = ndgrid(1:5,1:5);
                                centre_x=3;
                                centre_y=3;
                                d_predict = ((Y_mini - centre_y).^2 + (X_mini - centre_x).^2 ).^0.5;
...
%                             % If the last detection is not the same as
%                             % the current maxpos, replace it
%                             if(exist('last_detection','var'))
% %                                 if(maxpos.v ~= last_detection.v && maxpos.h ~= last_detection.h)
%                                     last_detection.v = maxpos.v;
%                                     last_detection.h = maxpos.h;
%                                     last_detection.tix = integrated_tix;
% %                                 end
