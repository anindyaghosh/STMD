...
function TrackerModel19_09_16(param_settings_file, gind, groupname, hours_to_run, in_toc)
disp(['Now running g: ' num2str(gind)]);
rng(0); % Fixed seed
% Because the seed is fixed, the distractor movements will always be the
% same. Therefore, not necessary to save this information out.

% To be provided via function call
% tic
% in_toc = toc;
% gind=1;
% data_dir = 'E:\PHD\conf2\data\results\';
% hours_to_run = 6/60;
% param_settings_file = 'E:\PHD\conf2\data\trackerinfo\TrackerParamFile19_09_14.mat';
...
toc_to_die = in_toc + hours_to_run*60*60 - 900;
...
settings.hfov = param_settings.fixed.hfov;
settings.vfov = param_settings.fixed.vfov;
settings.Ts = param_settings.fixed.Ts;
settings.saccade_duration = param_settings.fixed.saccade_duration;
...
settings.image_name = param_settings.fixed.image_name;
...
outer_ix=param_settings.outer_ix(gind);
inner_ix=param_settings.inner_ix(gind);

settings.outer_ix = outer_ix;
settings.inner_ix = inner_ix;

settings.tracker = param_settings.outer_var.sets.tracker{outer_ix};
distractorparams.draw_distractors = param_settings.outer_var.sets.draw_distractors(outer_ix);
settings.do_predictive_turn = param_settings.outer_var.sets.do_predictive_turn(outer_ix);
settings.do_saccades = param_settings.outer_var.sets.do_saccades(outer_ix);

if(strcmp(settings.tracker,'prob'))
    settings.output_integration = param_settings.params_set.prob.output_integration(inner_ix);
    facparams=[];
    % Tracker parameters
    trackerparams.pos_std_scalar = param_settings.params_set.prob.pos_std_scalar(inner_ix);
    trackerparams.output_threshold_scalar = param_settings.params_set.prob.output_threshold_scalar(inner_ix);
    trackerparams.vel_std_scalar = param_settings.params_set.prob.vel_std_scalar(inner_ix);
    trackerparams.acquisition_threshold = param_settings.params_set.prob.acquisition_threshold(inner_ix); % What the max probability has to evaluate out to before we assume a target is acquired
    trackerparams.turn_threshold = param_settings.params_set.prob.turn_threshold(inner_ix);
    trackerparams.reliability = param_settings.params_set.prob.reliability(inner_ix); % An assumption about how likely the measurement is to correspond to the target
    trackerparams.lost_position_threshold = param_settings.params_set.prob.lost_position_threshold(inner_ix);
    trackerparams.min_turn_gap = param_settings.params_set.prob.min_turn_gap(inner_ix);
    trackerparams.tix_ignore_scalar = param_settings.params_set.prob.tix_ignore_scalar(inner_ix);
    trackerparams.likelihood_threshold = param_settings.params_set.prob.likelihood_threshold(inner_ix);
...
elseif(strcmp(settings.tracker,'facilitation'))
    settings.output_integration = param_settings.params_set.fac.output_integration(inner_ix);
    trackerparams=[];
    
    facparams.gain = param_settings.params_set.fac.gain(inner_ix);
    facparams.sigma = param_settings.params_set.fac.sigma(inner_ix);
    facparams.kernel_spacing = param_settings.params_set.fac.kernel_spacing(inner_ix);
    facparams.emd_timeconstant = param_settings.params_set.fac.emd_timeconstant(inner_ix);
    facparams.lpf_timeconstant = param_settings.params_set.fac.lpf_timeconstant(inner_ix);
    facparams.distance_turn_threshold = param_settings.params_set.fac.distance_turn_threshold(inner_ix);
    facparams.direction_predict_distance = param_settings.params_set.fac.direction_predict_distance(inner_ix);
    facparams.direction_predict_threshold = param_settings.params_set.fac.direction_predict_threshold(inner_ix);
    facparams.direction_turn_threshold = param_settings.params_set.fac.direction_turn_threshold(inner_ix);
    facparams.tix_between_saccades = param_settings.params_set.fac.tix_between_saccades(inner_ix);
...
distractorparams.assumed_fwd_vel = param_settings.params_set.distractors.assumed_fwd_vel; % m/s
            ...
            if(mod(tix,settings.output_integration)==0) % Time to update the tracker or histograms as the case may be
                                ...
                                for pix_ix=1:numel(prob_R_pos) % For each actual pixel
                                    if(dark_output_integ{1}(pix_ix) < trackerparams.output_threshold)
                                        ...
                                        for p=1:numel(prob_R_pos_kernel)
                                            vmask = trackerparams.pos_velocities{p}; % Mask for the velocities which could be used to move from position p from the position currently being looked at
                            ...
                            if(acquired  && max_certainty > trackerparams.turn_threshold && (integrated_tix - last_turn_integrated) > trackerparams.min_turn_gap && ~executing_saccade && (tix - saccade_end_tix) > settings.tix_after_saccade)
                                ...
                                elseif(max_x_screen < settings.hfov/3)
                                    ...
                                    if(settings.do_predictive_turn)
                                        if(settings.do_saccades)
                                            predict_time = max(trackerparams.tix_ignore_after_turn * settings.output_integration*settings.Ts,...
                                                settings.saccade_duration+tix_after_saccade*Ts);
                                            predict_angle = predict_time * 60;
                                        else
                                            predict_angle = trackerparams.tix_ignore_after_turn * settings.output_integration*settings.Ts * 60;
                                        end
                ...
                if(settings.showimagery && tix > spin_ticks)
                    ...
                    if(settings.use_prob_tracker)
                        ...
                        subplot(4,2,5)
                        plot(logging.max_certainty_history(~isnan(max_certainty_history)))
                        if(acquired)
                            title('acquired')
                        else
                            title('not acquired')
