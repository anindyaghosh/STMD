function Model2018Script_WIDE_18_09_16(grp_index,grp_file,groupname)
...
%############
% grp_index=56618;
% grp_file='E:\PHD\CONFdronefiles\CONF_DroneHighRes18_08_30.txt';
...
success_threshold = (target_window^2+target_window^2)^0.5+0.1;
...
%         frame_buffer(:,:,framebuff_ind)=frame_buffer(:,:,framebuff_ind)/255;
    ...
    if(flag_for_record)
        ...
        if(ssgt.gtbuttonfill(abs_frame_num) == 1)
            ...
            for k_x=-success_threshold:success_threshold
                for k_y=-success_threshold:success_threshold
...
%             targresp.LIGHT(vid_framenum-1)=max(max(blight(max(ssgt.gty_ss(abs_frame_num)-1,1):min(1+ssgt.gty_ss(abs_frame_num),vfov),...
%                 max(1,ssgt.gtx_ss(abs_frame_num)):min(hfov,1+ssgt.gtx_ss(abs_frame_num)))));
%             targresp.DARK(vid_framenum-1)=max(max(bdark(max(ssgt.gty_ss(abs_frame_num)-1,1):min(1+ssgt.gty_ss(abs_frame_num),vfov),...
%                 max(1,ssgt.gtx_ss(abs_frame_num)):min(hfov,1+ssgt.gtx_ss(abs_frame_num)))));
%             targresp.COMBO(vid_framenum-1)=max(max(bcombo(max(ssgt.gty_ss(abs_frame_num)-1,1):min(1+ssgt.gty_ss(abs_frame_num),vfov),...
%                 max(1,ssgt.gtx_ss(abs_frame_num)):min(hfov,1+ssgt.gtx_ss(abs_frame_num)))));
        end
        ...
        if(ssgt.gtbuttonfill(abs_frame_num) == 1)
            bg_mask(max(ssgt.gty_ss(abs_frame_num)-1,1):min(1+ssgt.gty_ss(abs_frame_num),vfov),...
                max(1,ssgt.gtx_ss(abs_frame_num)):min(hfov,1+ssgt.gtx_ss(abs_frame_num)))=false;
        end
...
save([dataDir 'gind' num2str(grp_index) '.mat'],...
    ...
    'fdsr_lp','targresp','bg_counts','bg_edges')
