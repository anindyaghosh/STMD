...
elseif(strcmp(which_pc,'mecheng'))
    ...
    load('D:\temp\temp_lost_it_19_10_07.mat');
...
for tr_ix = 1:numel(track_ix)
    f=figure(tr_ix);
    f.Name = ['Target vel prior to failure ' num2str(track_ix(tr_ix))];
...
for hfov_setting = [40 60 90];
    for predictive_setting = [false true];
        ...
        for tr_ix = 1:numel(track_ix)
            f=figure(tr_ix);
            f.Name = ['hfov=' num2str(hfov_setting) 'pred=' num2str(predictive_setting) '-Saccades and distractors ' num2str(track_ix(tr_ix))];
            eb_med = NaN(3,4);
            eb_upper = eb_med;
            eb_lower = eb_med;
            for x=2:4
