...
% 19_10_30: Support completed for ZB_fac

function TrackerModel19_11_01(param_settings_file, gind, groupname, hours_to_run, in_toc)
disp(['Now running g: ' num2str(gind)]);

% % To be provided via function call
% clear all
% close all
% settings.ZBfac_noremap = true;
% tic
% in_toc = toc;
% gind=1;
% % data_dir = 'D:\temp\';
...
% param_settings_file = 'E:\PHD\TEST\TrackerParamFile19_11_01.mat';
% % E:\PHD\TEST
% groupname='something';
% % 
% which_pc='mrblack';
...
settings.ignore_after_turn_zbfac = param_settings.fixed.ZB_fac.ignore_after_turn_zbfac;
...
%     ZB_fac.output_threshold = param_settings.ZB_fac.output_threshold;
    ...
    settings.output_integration = param_settings.fixed.ZB_fac.output_integration;
    ZB_fac.output_threshold = param_settings.fixed.ZB_fac.output_threshold;

    ZB_fac.wb = param_settings.fixed.ZB_fac.wb;
    ZB_fac.FacilitationGain = param_settings.fixed.ZB_fac.FacilitationGain; %root of facilitation kernel gain
    ZB_fac.RC_wb = param_settings.fixed.ZB_fac.RC_wb; 
    ZB_fac.parameter.RC_fac_wb = param_settings.fixed.ZB_fac.parameter.RC_fac_wb;
    ZB_fac.Threshold = param_settings.fixed.ZB_fac.Threshold;
    ZB_fac.Ts = param_settings.fixed.ZB_fac.Ts;
...
%                            end
                            ...
                            % Has the effect of ignoring the detection if a
                            % saccade has recently occurred
                            if(settings.ignore_after_turn_zbfac)
                                if( (tix-saccade_end_tix) <= settings.tix_after_saccade || executing_saccade)
                                    default=0;
                                end
                            end
