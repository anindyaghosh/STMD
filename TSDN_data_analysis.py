...
stimulus = scipy.io.loadmat(os.path.join(data_folder, 'extractions', f'{stim_type}_background.mat'))
...
background_right = culling_stims(np.vstack(background_right)[:,2:])
background_left = culling_stims(np.vstack(background_left)[:,2:])
...
# sdfs = match_check([alone, stationary, background_right, background_left])

# alone = alone[[0, 1, 2, 4, 5, 6, 9, 10],:]
# stationary = stationary[[1, 3, 4, 6, 7, 8, 9],:]
# background_right = background_right[[0, 2, 5, 6, 9, 10],:]
# background_left = background_left[[1, 2, 4, 5],:]

sliding_window_length = int(bin_size * params['sf'] / 1000)

labels = ['Alone (no background)', 'Stationary background', 'Background right', 'Background left']
...
if not os.path.isfile(f'sdfs_{stim_type}.pkl'):
    ...
    pickle.dump(averages_all, open(f'sdfs_{stim_type}.pkl', 'wb'))
else:
    averages_all = pickle.load(open(f'sdfs_{stim_type}.pkl', 'rb'))
...
def moving_average_viz(moving_avgs):
    ...
    ewma_averages = dirac_lpf([alone, stationary, background_right, background_left])
...
spike_train_viz([alone, stationary, background_right, background_left], mode='spikes')